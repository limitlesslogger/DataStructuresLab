...
void arrangetranSparse(triplet t[],int n)
{
    triplet temp[1];
    for(int i=1;i<=n;i++)
    {
        for(int j=i+1;j<=n;j++)
        {
            if((t[j].rows<t[i].rows)||(t[j].rows==t[i].rows&&t[j].columns<t[i].columns))
            {
                // if(t[j].rows>t[j+1].rows)
                // {
                    temp[0]=t[i];
                    t[i]=t[j];
                    t[j]=temp[0];
            ...
            //else if(t[j].rows==t[i].row&&t[j].columns<t[i].columns)
        }
    }
}
void main()
